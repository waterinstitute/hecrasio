...
def pull_result_paths_s3(model):
    """
    Original script for extracting information from zipped folder of results.
    """
...
def pull_result_paths(files):
    """
    Returns the HDF file of the model plan
    """
    assert len([f for f in files if '.hdf' in f]) != 0, "No hdf files found"
    hdfFiles = []
    for c in files:
        filename = os.path.splitext(c)[0]
        extension = os.path.splitext(c)[1]
        if extension == '.hdf': 
            hdftype = filename.split('.',1)[1]
            if hdftype.startswith('p'): # only get plan hdf and not geometry hdf or other hdf files
                hdfFiles.append(c)
    return hdfFiles
    
def list_files(path):
    """ 
    Identify all files within a zip file or base directory of a folder
    """
    if '.zip' in path:
        """ unpack files from zip """
        zip_file = zipfile.ZipFile(open(path,"rb"))   
        contents = [x.filename for x in zip_file.infolist()]
    elif os.path.isdir(path):
        """ grab files in root directory """
        contents = [f for f in os.listdir(path) if os.path.isfile(os.path.join(path, f))]
    else:
        """ store file directly """
        contents = [path]
    return contents

def list_vol_errors(rasPlans):
    '''
    List the volume accounting errors of the models
    '''
    for rasPlan in rasPlans:    
        try: 
            errorpercent =(round(float(rasPlan.vol_acct_error), 3)) 
            color="green" 
            if errorpercent >1 and errorpercent<=3: 
                color="orange" 
            elif errorpercent > 3: 
                color="red" 
            display(HTML('<p style="font-size:18px"> <strong> Model </strong>: {0} <br>\
                            <strong>Volume Accounting Error: </strong> <span style="font-size:18px; font-weight:bold; color:{2}"> {1}%</span>  </p>' \
                            .format(rasPlan._name, errorpercent, color))) 
        except : 
            error = 'Vol Accounting Error Percentage can not be found for '+ rasPlan._name 
            print(error)
    return None